    ...
    while(true) {
      try {
        ...
        while (req == null) {
          ...
          req = requestChannel.receiveRequest(300) // 从RequestQueue中拉取请求
        ...
        // kafka Apis 类中实现了处理请求的逻辑 kafkaApis 还负责将响应写会对应的processor 也就是 对应的 ResponseQueue
        apis.handle(req) // 对应消息处理
      } catch {
...
class KafkaRequestHandlerPool(val brokerId: Int,
  ...
  def shutdown() {
    info("shutting down") // 停止Threads 线程
    for(handler <- runnables)
      handler.shutdown
    for(thread <- threads) // 阻塞等待所有KafkaRequestHandler线程结束
      thread.join
