...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      ApiKeys.forId(request.requestId) match {// 根据 requestId 分发请求
          // ApiKeys.PRODUCE 表示ProducerRequest
  ...
  def handleProducerRequest(request: RequestChannel.Request) {
    ...
    // partitionRecords -> Map<TopicPartition, ByteBuffer>
     // partition 将map 分成两份 一份鉴权的 一份不鉴权的 \
    // asScala 隐式转换 import scala.collection.JavaConverters._  两个Map
