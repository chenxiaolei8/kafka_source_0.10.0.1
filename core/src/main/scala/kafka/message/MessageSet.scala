 ...
 * as follows:
 * 8 byte message offset number 8字节的 offset
 * 4 byte size containing an integer N  4个字节的消息大小
 * N message bytes as described in the Message class N 个字节的消息
 */
abstract class MessageSet extends Iterable[MessageAndOffset] {
   ...
   * Provides an iterator over the message/offset pairs in this set
    * 提供迭代器 保证其顺序性 和writeTo 组合在一起保证 .log文件的顺序写与读  其中涉及到读取压缩消息
   */
