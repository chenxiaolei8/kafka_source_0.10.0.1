...
object ByteBufferMessageSet {
  ...
  private def create(offsetAssigner: OffsetAssigner,
    ...
    // 如果传入的是null 则返回空的ByteBuffer
    if (messages.isEmpty)
      MessageSet.Empty.buffer
    // 如果要求部队信息不进行压缩
    // 则通过OffsetAssigner分配每个消息的offset， 再讲消息写入到 ByteBuffer 之后 返回ByteBuffer.
    else if (compressionCodec == NoCompressionCodec) {
      val buffer = ByteBuffer.allocate(MessageSet.messageSetSize(messages))
      for (message <- messages) writeMessage(buffer, message, offsetAssigner.nextAbsoluteOffset())
      buffer.rewind()
      buffer
      // 如果要求 对消息进行压缩 首先将消息 按照指定的压缩模式进行压缩并保存到缓冲区 同时也会完成offset的分配，然后按照压缩消息的格式
      // 写入外层消息 然后将整个外层消息所在的 ByteBuffer返回
    } else {
      // 得到 Magic 和时间戳
      ...
      messageWriter.write(codec = compressionCodec, timestamp = magicAndTimestamp.timestamp, timestampType = timestampType, magicValue = magicAndTimestamp.magic) { outputStream =>
        ...
        try {
          for (message <- messages) {
            ...
            // magic =1 写入的是相对offset， magic = 0 写入的是offset
            if (magicAndTimestamp.magic > Message.MagicValue_V0)
              output.writeLong(offsetAssigner.toInnerOffset(offset)) // 相对offset
            else
...
/**
  ...
  * 压缩消息没有 key = null
  * 压缩消息长度  offset + size + CRC32 + magic + attributes +timestamp + keyLength + valueLength + value
  */
class ByteBufferMessageSet(val buffer: ByteBuffer) extends MessageSet with Logging {
  ...
  def writeFullyTo(channel: GatheringByteChannel): Int = {
    buffer.mark()
    var written = 0
    while (written < sizeInBytes) // ByteBufferMessageSet 中的数据全部写入文件
