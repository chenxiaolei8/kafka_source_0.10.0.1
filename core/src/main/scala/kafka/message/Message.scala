...
object Message {
   ...
   */
  val CrcOffset = 0
  val CrcLength = 4 // crc32
  val MagicOffset = CrcOffset + CrcLength
  val MagicLength = 1 // 魔术指数
  val AttributesOffset = MagicOffset + MagicLength
  // 一个字节 八位 0-3表示压缩模式 0 无压缩 1 gzip压缩 2 snappy压缩 3 lz4压缩 4表示时间戳类型 0 表示创建时间 1表示追加时间
...
class Message(val buffer: ByteBuffer,
  ...
  def this(bytes: Array[Byte], 
    ...
    /**
      * 1字节 8位
      * CRC32 4字节 消息的检验码
      * magic 1字节 魔数标示 与消息格式有关 取值0或1 当magic为0时 消息的offset使用绝对值且消息格式中没有timestamp部分 当magic为1时 消息的
      *       Offset使用相对值且消息格式中存在timestamp部分 所以magic的不同 消息的长度不一样
      * attributes 1字节 消息的属性 其中第0 - 2位的组合表示消息使用的压缩模式 0 表示无压缩 1 表示gzip 2 表示snappy 3表示lz4
      *             第3位表示时间戳类型 0表示创建时间 1表示追加时间 4 - 7保留
      * timestamp 根据 magic的值表示 如果有 则根据attribute中的第3位确定   统一长度是8位
      * key length 消息key的长度
      * key 消息的key
      * value length 消息的长度
      * value 消息
