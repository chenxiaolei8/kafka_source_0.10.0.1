 ...
 */
@nonthreadsafe
class FileMessageSet private[kafka](@volatile var file: File, // file  指磁盘上对应的日志文件
                                    private[log] val channel: FileChannel,// 用于 读写对应的文件
                                    private[log] val start: Int, // start and end 表示分片的 起始和结束的位置
   ...
   * If it's new file and preallocate is true, end will be set to 0.  Otherwise set to Int.MaxValue.
    * preallocate
   */
  def this(file: File, fileAlreadyExists: Boolean, initFileSize: Int, preallocate: Boolean) = // preallocate 是否开始预分配
      this(file,
  ...
  def searchFor(targetOffset: Long, startingPosition: Int): OffsetPosition = {
    var position = startingPosition // 当前 下标
    val buffer = ByteBuffer.allocate(MessageSet.LogOverhead) // 12 字节
    val size = sizeInBytes() // FileMessageSet 大小 按照字节 整个 fileMessageSet的大小
    while(position + MessageSet.LogOverhead < size) { // 包含
      buffer.rewind() // 重置ByteBuffer的position 准备读入数据
      // 读取LogOverhead 这里会确保从StartingPositing
      channel.read(buffer, position) // 读取 offset and 消息的长度
      if(buffer.hasRemaining)
        throw new IllegalStateException("Failed to read complete buffer for targetOffset %d startPosition %d in %s"
                                        .format(targetOffset, startingPosition, file.getAbsolutePath))
      // 重置ByteBuffer的position 准备读出数据
      buffer.rewind()
      val offset = buffer.getLong()
      if(offset >= targetOffset)
        return OffsetPosition(offset, position)
      val messageSize = buffer.getInt() // 消息的大小
      if(messageSize < Message.MinMessageOverhead)
        throw new IllegalStateException("Invalid message size: " + messageSize)
      position += MessageSet.LogOverhead + messageSize // 找到下一条 信息
    }
    null
  }
  ...
  /**
    * 裁剪文件 切换leader的时候 需要 比如 ack 确认副本同步到哪了
  ...
  def truncateTo(targetSize: Int): Int = {
    val originalSize = sizeInBytes // 记录 原始记录数
    if(targetSize > originalSize || targetSize < 0) // 排除 脏数据
      throw new KafkaException("Attempt to truncate log segment to " + targetSize + " bytes failed, " +
                               " size of this log segment is " + originalSize + " bytes.")
    if (targetSize < channel.size.toInt) {
      channel.truncate(targetSize) //裁剪
      channel.position(targetSize) // 更新 最新 记录数
      _size.set(targetSize) // 设置文件大小
    }
    originalSize - targetSize // 返回 删掉的裁剪数
  }
...
object FileMessageSet
{
  ...
  def openChannel(file: File, mutable: Boolean, fileAlreadyExists: Boolean = false, initFileSize: Int = 0, preallocate: Boolean = false): FileChannel = {
    if (mutable) {// 创建的fileChannel是否可写
      if (fileAlreadyExists)
        new RandomAccessFile(file, "rw").getChannel()
      else {
        if (preallocate) { // 进行文件预分配
          // 创建 可读写的文件channel 有最大值 initFileSize
