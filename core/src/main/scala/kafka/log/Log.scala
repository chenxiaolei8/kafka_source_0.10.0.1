...
class Log(val dir: File,
  ...
  def append(messages: ByteBufferMessageSet, assignOffsets: Boolean = true): LogAppendInfo = {
    ...
    // trim any invalid bytes or partial messages before appending it to the on-disk log 清除未验证通过的消息
    ...
    try {
      ...
      lock synchronized {
        ...
        if (assignOffsets) {   // 判断需要分配offset 默认需要
          // assign offsets to the message set
          val offset: LongRef = new LongRef(nextOffsetMetadata.messageOffset)
  ...
  private def analyzeAndValidateMessageSet(messages: ByteBufferMessageSet): LogAppendInfo = {
    ...
    var monotonic = true // offset是否内存单调递增
    for(messageAndOffset <- messages.shallowIterator) {
      ...
      // check the validity of the message by checking CRC32
      m.ensureValid()

      shallowMessageCount += 1
      validBytesCount += messageSize

      val messageCodec: CompressionCodec = m.compressionCodec
      if(messageCodec != NoCompressionCodec)
        sourceCodec = messageCodec
    }

    // Apply broker-side compression if any
    val targetCodec: CompressionCodec = BrokerCompressionCodec.getTargetCompressionCodec(config.compressionType, sourceCodec)
  ...
  private def trimInvalidBytes(messages: ByteBufferMessageSet, info: LogAppendInfo): ByteBufferMessageSet = {
    val messageSetValidBytes: Int = info.validBytes
  ...
  private def maybeRoll(messagesSize: Int): LogSegment = {
    ...
    if (segment.size > config.segmentSize - messagesSize || // 当前activeSegment的日志大小加上本次带追加的消息集合大小 超过配置的logSegment的最大长度
        segment.size > 0 && time.milliseconds - segment.created > config.segmentMs - segment.rollJitterMs || // 当前activeSegment的寿命超过了配置的logSegment生命周期
        segment.index.isFull) { // 满了
