 ...
 * storage format.
  * _file 指向磁盘的索引文件
  * baseOffset 对应日志文件中第一个offset
  * mmap 用来操作索引文件的MapperByteBuffer
  * lock 多线程访问mmap 加锁
  * _entries 当前索引文件中的个数
  * _maxEntries 当前文件最多存储的索引数
  * _lastOffset 保存当前文件的最后一个索引的offset
...
class OffsetIndex(@volatile private[this] var _file: File, val baseOffset: Long, val maxIndexSize: Int = -1) extends Logging {
  ...
  private[this] var mmap: MappedByteBuffer = {
    ...
    try {
      ...
      if (newlyCreated) {
        ...
        raf.setLength(roundToExactMultiple(maxIndexSize, 8)) // 大小为8的倍数
      }
  ...
  def lookup(targetOffset: Long): OffsetPosition = {
    maybeLock(lock) {
      ...
      // 二分查找
      val slot: Int = indexSlotFor(idx, targetOffset)
      if(slot == -1)
  ...
  private def indexSlotFor(idx: ByteBuffer, targetOffset: Long): Int = {
    ...
    // check if the target offset is smaller than the least offset 最小值都比你大
    if (relativeOffset(idx, 0) > relOffset)
      return -1
    // 常规二分法查找index
    // binary search for the entry
    var lo = 0
    var hi = _entries - 1 // 索引数
    while (lo < hi) {
