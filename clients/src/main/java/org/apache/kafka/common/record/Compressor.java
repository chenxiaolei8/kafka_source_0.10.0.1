...
public class Compressor {
    ...
    private final DataOutputStream appendStream;  // 装饰
    ...
    public Compressor(ByteBuffer buffer, CompressionType type) {
        ...
        if (type != CompressionType.NONE) {
            ...
            buffer.position(initPos + Records.LOG_OVERHEAD + Record.RECORD_OVERHEAD); // 初始位置增加 （必要的预留参数）
        }
        // create the stream
    ...
    public void close() {
        ...
        if (type != CompressionType.NONE) { // 当前确定 使用压缩算法
            ByteBuffer buffer = bufferStream.buffer();
            int pos = buffer.position(); // 保存buff的尾部位置
            // write the header, for the end offset write as number of records - 1
            buffer.position(initPos); // 将指针 移到初始位置
            buffer.putLong(numRecords - 1); // 当前消息数量
            buffer.putInt(pos - initPos - Records.LOG_OVERHEAD); // 除了 offset 大小 and 消息的大小
