...
public class NetworkClient implements KafkaClient {
    ...
    /* the state of each node's connection 维护了 自己的map */
    private final ClusterConnectionStates connectionStates;

    /* the set of requests currently being sent or awaiting a response 发送出去 但并未收到回复的请求 当然 ack = 0 发送出去后 直接删除对应的数据*/
    ...
    public NetworkClient(Selectable selector, // 封装了 selector （封装了 nioSelector）
                         Metadata metadata,
                         String clientId,
    ...
    private NetworkClient(MetadataUpdater metadataUpdater,
        ...
        if (metadataUpdater == null) {
            ...
            this.metadataUpdater = new DefaultMetadataUpdater(metadata); // 内部类
        } else {
        ...
        this.correlation = 0;   // 相关性 目前我没解释  这只是 字面意思
    ...
    public boolean ready(Node node, long now) {
        if (node.isEmpty())  // 筛选 node的 名字
            throw new IllegalArgumentException("Cannot connect to empty node " + node);

        if (isReady(node, now)) // metadata 未更新 并且 目的端 负载还能发送 Request
            return true;

        if (connectionStates.canConnect(node.idString(), now))
            // if we are interested in sending to a node and we don't have a connection to it, initiate one
            initiateConnect(node, now); // 初始化node连接
    ...
    public boolean isReady(Node node, long now) {
        ...
        // priority // metadata 处于未更新的状态  并且 改node 能够发送更多消息
     ...
     * @param node The node
     */
    private boolean canSendRequest(String node) {    // selector.isChannelReady(node)  -->  永远返回的true
     ...
     * @param now     The current timestamp
     */
    @Override  // send -> doSend -> send -> setSend
    /**
     * 最终放入KafkaChannel中的send  并OP_WRITE
     */
    ...
    private void doSend(ClientRequest request, long now) {
        ...
        // 添加到 inFlight
        this.inFlightRequests.add(request); //实际上 这个 存储的 都是发或者 正在发出去的请求 收到Response后 对比 ack 删除即可 如果 不用回调 也是直接删除
        selector.send(request.request()); // requestSend 头信息 and body
    }

    /**
     * Do actual reads and writes to sockets.                                                                                                                                                吧
     * v
     *
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        long metadataTimeout = metadataUpdater.maybeUpdate(now);  // 发送metadata更新请求 查看 metadata 更新时间 并且 未完成连接的 也在这里 创建连接
        try {
            this.selector.poll(Utils.min(timeout, metadataTimeout, requestTimeoutMs));  // 发送请求
        } catch (IOException e) {
        ...
        // 一系列的handle方法 调用返回信息
        handleCompletedSends(responses, updatedNow); // 发送成功的 加入 responses
    ...
    private void handleCompletedSends(List<ClientResponse> responses, long now) {
        ...
        for (Send send : this.selector.completedSends()) { // poll write事件 写出后 send 是存在的
            ClientRequest request = this.inFlightRequests.lastSent(send.destination());
            if (!request.expectResponse()) { // 发现不需要 响应的请求 这里对应的事 ack 设置 也就是 acks = 0 不用 expect 期望
                this.inFlightRequests.completeLastSent(send.destination()); // 删除 当前已经发送成功的send 并加入responses
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            if (!metadataUpdater.maybeHandleCompletedReceive(req, now, body))// 如果不是metadata的相应测 加入到response 如果是则更新metadata 并notify
    ...
    private void initiateConnect(Node node, long now) {
        ...
        try {
            ...
            metadataUpdater.requestUpdate(); // 请求更新 metadata
            log.debug("Error connecting to node {} at {}:{}:", node.id(), node.host(), node.port(), e);
        }
    }

    class DefaultMetadataUpdater implements MetadataUpdater { // metadata 管理类 发送请求 已经更新metadata

        /* the current cluster metadata */
        private final Metadata metadata;

        /* true iff there is a metadata request that has been sent and for which we have not yet received a response */
        // 用来标示是否已经发送 更新信息 避免重新发送
        ...
        DefaultMetadataUpdater(Metadata metadata) {
            ...
            this.metadataFetchInProgress = false; // 更新请求中  这个时候 会标示为 true
        ...
        @Override
        public boolean isUpdateDue(long now) { // 是否更新时间已到   判断规则: 没有在更新中 且下次更新时间已到
            return !this.metadataFetchInProgress && this.metadata.timeToNextUpdate(now) == 0; // 涉及到的 规避时间 也要做处理 已做
        }
        ...
        public long maybeUpdate(long now) {
            ...
            // 检测是否发生了MetadataFetchInProgress
            ...
            if (metadataTimeout == 0) {
                ...
                Node node = leastLoadedNode(now); // 找到负载最小的 节点
                maybeUpdate(now, node); // 添加一个 metaDataUpdate请求
            }
        ...
        @Override
        public boolean maybeHandleCompletedReceive(ClientRequest req, long now, Struct body) {// 李婷婷生日快乐
        ...
        private void maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId)) {
                ...
                doSend(clientRequest, now); //请求 放入 待发送中 下次 更新 metadata
            ...
            } else { // connected, but can't send more OR connecting  已连接状态 但是不能在发送更多消息（上线） 其中任何一种状态 我们都需要等待network一段时间
