...
public class RequestFuture<T> {
     ...
     */
    // 适配器模式 adapter他就是一个适配器
    // 调用过程 回调时的调用过程 也可以认为是请求完成的事件传播流程   当条用RequestFuture<T> 对象的complete() 或者raise()
    // 方法时 回调用 RequestFutureListener<T> 的onSuccess() 或 onFailure() 方法 然后调用RequestFutureAdapter<T,S>的对应方法 最终调用
    // RequestFuture 的对应方法
