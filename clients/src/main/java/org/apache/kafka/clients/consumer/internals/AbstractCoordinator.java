...
public abstract class AbstractCoordinator implements Closeable {
    ...
    /**
     * 阻塞线程直到group的coordinator准备好 并 回应request请求
     * Block until the coordinator for this group is known and is ready to receive requests.
     */
    public void ensureCoordinatorReady() {
        while (coordinatorUnknown()) { // 1 确定groupCoordinator状态 返回是否为空 进入send poll
            // 2 创建并缓存请求
            RequestFuture<Void> future = sendGroupCoordinatorRequest();
            // 阻塞发送GroupCoordinatorRequest， 并处理响应GroupCoordinatorResponse
            client.poll(future);
            if (future.failed()) { // 3 异常处理
                if (future.isRetriable())
                    client.awaitMetadataUpdate();
                else
                    throw future.exception();
            } else if (coordinator != null && client.connectionFailed(coordinator)) { //4 coordinator的node 连接失败 重置coordinator
    ...
    private RequestFuture<Void> sendGroupCoordinatorRequest() {
        ...
        Node node = this.client.leastLoadedNode(); // 获取负载最小的节点 底层就是查询InFlightRequest中未确定请求最少的节点
        if (node == null) {
    ...
    private void handleGroupMetadataResponse(ClientResponse resp, RequestFuture<Void> future) {
        ...
        if (!coordinatorUnknown()) {// 因为没有做防止重复发送的机制 所以 可能会发送多个链接 那么当有一个Response得到响应 其他的Response就忽略掉
            // We already found the coordinator, so ignore the request
            future.complete(null);
        } else {
            // 解析Response 得到服务端的GroupCoordinator信息
    ...
    public boolean coordinatorUnknown() {
        ...
        if (client.connectionFailed(coordinator)) {
            coordinatorDead(); // coordinator 值为null
            return true;
        }
