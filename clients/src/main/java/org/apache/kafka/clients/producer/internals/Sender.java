...
public class Sender implements Runnable {
    ...
    private volatile boolean forceClose;  // 线程值改变后 立马 写到主内存
     ...
     */
    void run(long now) {
        Cluster cluster = metadata.fetch(); // 1 fetch() 2 返回cluster
        // get the list of partitions with data ready to send
        // 预备动作 转可发送节点  broker
        RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now); // 3 ready() 4 返回ReadyCheckResult
        // if there are any partitions whose leaders are not known yet, force metadata update
        if (result.unknownLeadersExist) // 根据结果 更新元数据
            this.metadata.requestUpdate(); //5 needUpdate = true
        ...
        while (iter.hasNext()) {
            Node node = iter.next();
            if (!this.client.ready(node, now)) { // 反复筛选 可用节点 并创建传输层 开启 连接事件   这里 创建连接如果没有连接node的话
                iter.remove();
                notReadyTimeout = Math.min(notReadyTimeout, this.client.connectionDelay(node, now));
            }
        }
        // create produce requests   //转换动作 面向谁 发消息
        ...
        // 查看 里面recordBatch 消息 超时的 超时的 干掉
        ...
        // 创建回调对象 、 ProduceRequest 是每一个node 生成ClientRequest
        List<ClientRequest> requests = createProduceRequests(batches, now); // 每个 node 一个 ClientRequest
        ...
        // 这个时候 放进 inFlightRequest
        for (ClientRequest request : requests)
            client.send(request, now); // 缓存进 inFlightRequest 并且 发送修改KafkaChannel的send值以及传输层设置OP_WRITE

        // if some partitions are already ready to be seSnt, the select time would be 0;
    ...
    private void completeBatch(RecordBatch batch, Errors error, long baseOffset, long timestamp, long correlationId, long now) {
        if (error != Errors.NONE && canRetry(batch, error)) {
            ...
            // tell the user the result of their request  调用
            batch.done(baseOffset, timestamp, exception);
            // 释放资源
            this.accumulator.deallocate(batch); // 删除 incomplete 集合中的 batch  在新建batch 的时候 插入的 发送完成后 remove
     ...
     * 不管 发送到 那个 partition 总归 只能 每个node 只有一个请求 requestSend
     */
    private List<ClientRequest> createProduceRequests(Map<Integer, List<RecordBatch>> collated, long now) {
        List<ClientRequest> requests = new ArrayList<ClientRequest>(collated.size());
        for (Map.Entry<Integer, List<RecordBatch>> entry : collated.entrySet())
            // key 分resultSend entry.Key()
     ...
     * 这个batches 是对于一个node  destination
     */
    private ClientRequest produceRequest(long now, int destination, short acks, int timeout, List<RecordBatch> batches) {
        ...
        // 组装ProduceRequest 基于<topic<partition value >>按照API组装请求
        ProduceRequest request = new ProduceRequest(acks, timeout, produceRecordsByPartition);
        // node 请求头 消息体
        RequestSend send = new RequestSend(Integer.toString(destination),
                this.client.nextRequestHeader(ApiKeys.PRODUCE),
                request.toStruct());
        // 创建回调对象  请求 完成后      执行 Response   持有的另外一个kv备份对应
        RequestCompletionHandler callback = new RequestCompletionHandler() {
            public void onComplete(ClientResponse response) {
                // 查看状态值 调用completeBatch 完成 回调
